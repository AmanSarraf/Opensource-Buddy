(function() {
    fetch("CONTRIBUTORS.md").then((response) => response.text()).then((text) => {
        const container = document.querySelector("#contributors");        
        const list = text.split("\n").filter((_, ind) => ind > 0).map(el => el.replaceAll("- [", "").replaceAll(")", "").split("](").map(el => el.trim()));
        for(let [name, link] of list) {
            ...
            img.style = "height: 48px; width: 48px; border-radius: 100%; padding: 0;";
            ...
            swatch.style = "display: flex; align-items: center; padding: 0; justify-content: center;";
            ...
            contributor.style = "display: inline-block; background: #cc3880; color: #fff; margin: 10px; padding: 10px; border-radius: 10px; text-decoration: none;";
